...
def _parse_structured_content(content_div: BeautifulSoup, doc_id: str, url: str, doc_title:str):
    ...
    for pgroup_div in content_div.select('div.pgroup'):
        ...
        # title_tag가 없거나 내용이 없는 경우는 건너뜁니다.
        ...
        chunk_data = {
            ...
            "title": article_title,
            "text": final_text,
            "metadata": {"chapter": current_chapter or doc_title},
            "source_url": url
        }
        chunks.append(chunk_data)
    return chunks

def _parse_unstructured_content(content_div: BeautifulSoup, doc_id: str, url: str, doc_title: str):
    """'Ⅰ.', '1.' 등 비정형 고시 구조를 RAG에 최적화하여 파싱합니다."""
    chunks = []

    actual_content_container = content_div.select_one('.lawcon') if content_div.select_one('.lawcon') else content_div
    all_elements = actual_content_container.find_all(True, recursive=False)
    ...
    for section_elements in sections:
        ...
        is_main_titled_section = re.match(r'^\s*[ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ]+\.\s', first_el_text)

        if is_main_titled_section:
            title = first_el_text
            text = "\n".join(el.get_text(strip=True) for el in section_elements[1:])
            chunk_id = f"doc:{doc_id}:section_{chunk_index}"
            chunk_index += 1
        else:
            title = doc_title or "서문"
            text = "\n".join(el.get_text(strip=True) for el in section_elements)
            chunk_id = f"doc:{doc_id}:preamble"
...
def parse_law_html(html: str, url: str):
    ...
    doc_subtitle_tag = soup.select_one('div.subtit1')
    ...
    if not doc_id:
        logger.warning(f"URL에서 문서 ID(admRulSeq, admRulId)를 찾을 수 없습니다. 제목 기반으로 ID를 생성합니다. URL: {url}")
        doc_id = re.sub(r'[^a-zA-Z0-9]', '', doc_title)

    document_data = { "doc_id": doc_id, "title": doc_title, "subtitle": doc_subtitle, "source_url": url }

    content_div = soup.select_one('#contentBody, #conScroll, .lawcon')
    if not content_div:
    ...
    is_structured = any(
        ...
        for label in content_div.select('div.pgroup span.bl label')
    )

    if is_structured:
        logger.info("정형화된 법령 구조('제N조' 제목 발견)를 감지했습니다.")
        chunks = _parse_structured_content(content_div, doc_id, url, doc_title)
    else:
        logger.info("비정형적인 고시/공고 구조('제N조' 제목 미발견)를 감지했습니다.")
        chunks = _parse_unstructured_content(content_div, doc_id, url, doc_title)

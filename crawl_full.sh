...
# 크롤링 실행
if [ -z "$MAX_PAGES" ]; then
    .venv/bin/python3 crawl.py --scraper $SCRAPER --concurrent $CONCURRENT | tee "$LOG_FILE"
else
    .venv/bin/python3 crawl.py --scraper $SCRAPER --pages "$MAX_PAGES" --concurrent $CONCURRENT | tee "$LOG_FILE"
fi

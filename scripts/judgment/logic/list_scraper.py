...
def _parse_rows(html: str) -> List[dict]:
    ...
    for row in rows:
        ...
        safe_name = re.sub(r'[\\/*?:"<>|]', "", title).strip() if title else f"judgment_{jgmt_sn}"
...
async def fetch_urls(start_url: str, max_pages_arg: int | None = None):
    """
    주요판정사례 목록 페이지를 순회하며 상세 페이지 URL을 추출하여 반환합니다.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="주요판정사례 URL 수집기")
    parser.add_argument("--max_pages", type=int, default=None, help="최대 페이지 수")
    args = parser.parse_args()

    urls = asyncio.run(fetch_urls("https://nlrc.go.kr/nlrc/mainCase/judgment/index.do", args.max_pages))
    print(f"\n수집된 URL: {len(urls)}개")
